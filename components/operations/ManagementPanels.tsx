        ...
        <form action={action} key={editing?.id ?? "new"} className="ops-data-form">
          ...
          <label className="form-span-2">
            ...
            <input
              ...
              multiple
            />
            <small>Select one or multiple images. The first becomes the main image.</small>
          </label>
...
export function SuppliersPanel({ suppliers }: { suppliers: Supplier[] }) {
  ...
  return <div className="ops-content"><div className="ops-section-head"><div><p className="ops-kicker">Supplier directory</p><h2>Keep sourcing contacts reusable.</h2></div><span className="ops-badge">{suppliers.length} suppliers</span></div>
    <section className="ops-form-card"><div className="ops-form-title"><div><p className="ops-kicker">New supplier</p><h3>Add supplier</h3></div></div><form action={action} className="ops-data-form">
      ...
      <label><span>Country</span><input name="country" />
      </label>
      <label><span>City</span><input name="city" />
      </label><label><span>Email</span><input name="email" type="email" /></label>
      <label><span>Phone</span><input name="phone" /></label>
      <label><span>Website</span><input name="website" /></label>
      <label className="form-span-2"><span>Brands, comma separated</span><input name="brands" /></label>
      <label className="form-span-2"><span>Categories, comma separated</span><input name="categories" /></label>
      <label className="form-span-2"><span>Notes</span><textarea name="notes" rows={2} /></label>
      <div className="form-actions"><button className="ops-primary-button" disabled={pending}>{pending ? "Saving…" : "Add supplier"}</button>
      </div><ActionMessage state={state} />
    </form></section>
